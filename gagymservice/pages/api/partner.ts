...
const partnerApi = {
  get: () =>
    axios.get<PartnerItemResponse>(
       `http://3.35.145.146/partner`
    ),
  // axios.get<응답데이터의타입>(요청URL);
  // GET 요청URL HTTP/1.1
  fetch: () =>
  axios.get<PartnerItemResponse[]>(`http://3.35.145.146/partner`),
  ...
  remove:(id:number) =>
  axios.delete<boolean>(`http://3.35.145.146/partner/${id}`),

  modify :( id:number,partnerItem:PartnerItemRequest)=>
  axios.put<PartnerItemResponse>(
    `http://3.35.145.146/partner/${id}`,
    partnerItem
  )
};
